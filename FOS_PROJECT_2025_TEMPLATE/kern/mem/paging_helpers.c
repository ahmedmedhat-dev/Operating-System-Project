...
inline int  pt_get_page_permissions(uint32* directory, uint32 virtual_address )
{
	...
	int ret = get_page_table(directory,virtual_address,&page_table);
	if(ret != 0)
	{
		return -1 ;
	}

	uint32 index_inside_page_table = PTX(virtual_address);

	uint32 page = page_table[index_inside_page_table] ;
