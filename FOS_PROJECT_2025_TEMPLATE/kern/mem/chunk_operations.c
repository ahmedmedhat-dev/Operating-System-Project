...
//		return;
	...
	for(uint32 i = virtual_address; i < size + virtual_address ; i += PAGE_SIZE)
	{
		uint32 *page_table = NULL ;
		int ret = get_page_table(e->env_page_directory,i,&page_table);
		if(ret == TABLE_NOT_EXIST)
		{
			create_page_table(e->env_page_directory,i) ;
		}
		pt_set_page_permissions(e->env_page_directory, (uint32)i, PERM_UHPAGE, 0);
	}
#endif
}
...
void free_user_mem(struct Env* e, uint32 virtual_address, uint32 size)
{
	...
	for (uint32 it = virtual_address; it < virtual_address + size; it += PAGE_SIZE)
	{
		if (pt_get_page_permissions(e->env_page_directory, it) & PERM_PRESENT)
		{
			unmap_frame(e->env_page_directory, it);
			env_page_ws_invalidate(e, it);
		}
		pt_set_page_permissions(e->env_page_directory, it, 0, PERM_UHPAGE);
